...
class Andor:
    ...
    def StartAcquisition(self, wait_for_acquisition=True):
    ...
    def GetAcquiredDataAsArray(self):
        ...
        cimageArray = c_int * dim
        cimage = cimageArray()
        error = self.dll.GetAcquiredData(pointer(cimage), c_int(dim))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)

        cimage = np.array(cimage)
        cimage.reshape(shape)
        return cimage
